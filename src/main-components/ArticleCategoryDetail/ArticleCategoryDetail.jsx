...
const ArticleCategoryDetailPage = ({slug}) => {
    ...
    return (
      <div className={styles.pageContainer} style={{ textAlign: 'center' }}>
        <h1>Article Category Data Missing</h1>
        <Link href="/article-categories" className={styles.backLink}>
          &larr; Back to Article Categories
        </Link>
      </div>
    );
  }
   const categoryArticles = articlesData.filter(d => d.article_category_id === currentCategory.id);
   const segments = [
    
    { label: 'Article Categories', url: '/article-categories' },
  ...
  return (
    <div className={styles.pageContainer}>
        ...
        {categoryArticles.length > 0 ? (
          <ul className={styles.itemsGrid}>
            {categoryArticles.map(item => {
              const itemLink = `/articles/${item.slug}`;
              return (
          ...
          <p className={styles.noItemsText}>
            No articles currently listed for the "{currentCategory.name}" category.
          </p>
        )}
      </section>
      ...
      <div style={{ textAlign: 'center', marginTop: '40px' }}>
        <Link href="/article-categories" className={styles.backLink}>
          &larr; Explore Other Article Categories
        </Link>
      </div>
    </div>
  )
...
export default ArticleCategoryDetailPage